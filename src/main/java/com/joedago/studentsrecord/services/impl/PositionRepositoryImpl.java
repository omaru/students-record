...
public class PositionRepositoryImpl implements PositionRepository {
	...
	private URI getRequestUrl(final Student student) {
		final QueryUri queryUri = QueryUri.builder().firstAddress(student.getStreetAddress1())
				.secondAddress(student.getStreetAddress2()).build();
		final String address = queryUri.toString();
		return UriComponentsBuilder.fromHttpUrl(properties.getUrl()).path(properties.getForward())
				.queryParam(ACCESS_KEY, properties.getToken()).queryParam(QUERY, address).build().toUri();
	}
}
